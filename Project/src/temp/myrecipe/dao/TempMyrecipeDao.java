...
import temp.myrecipe.dto.TempMyrecipeDto;
import test.util.DbcpBean;

public class TempMyrecipeDao {
	private static TempMyrecipeDao dao;
	private TempMyrecipeDao() {}
	public static TempMyrecipeDao getInstance() {
		if(dao==null) {
			dao=new TempMyrecipeDao();
		}
		return dao;
	}
	//게시글 정보를 DB에 임시 저장하는 메소드
	public boolean temp_insert(TempMyrecipeDto dto) {
	...
	//임시 저장된 글이 있는지 확인하는 메소드
	public boolean temp_insert_Confirm(String writer) {
		//저장된 글이 있는 지 판별해주는 변수.
		...
		try {
			...
			conn = new DbcpBean().getConn();
			//실행할 sql 문 준비하기
			String sql = "SELECT tmp_num,tmp_title,tmp_subTitle,tmp_content,tmp_writer,tmp_regdate,tmp_imagePath,tmp_showImage,tmp_viewCount"
					+ " FROM tmp_my_recipe"
					+ " WHERE tmp_writer = ?";
			pstmt = conn.prepareStatement(sql);
	...
	//임시 저장된 글쓰기 정보를 수정하는 메소드
	public boolean temp_update(TempMyrecipeDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql 문 준비하기
			String sql = "UPDATE tmp_my_recipe"
					+ " SET tmp_title=?, tmp_subTitle=?, tmp_imagePath=?, tmp_showImage=?, tmp_content=?"
					+ " WHERE tmp_writer = ?";
			pstmt = conn.prepareStatement(sql);
			//? 에 바인딩 할 값이 있으면 바인딩한다.
			pstmt.setString(1, dto.getTitle());
			...
			pstmt.setString(4, dto.getShowImage());
	...
	//임시 저장된 글 하나의 정보를 삭제하는 메소드
	public boolean temp_delete(String writer) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql 문 준비하기 
			String sql = "DELETE FROM tmp_my_recipe"
					+ " WHERE tmp_writer=?";
			pstmt = conn.prepareStatement(sql);
			//? 에 바인딩 할 값이 있으면 바인딩한다.
			pstmt.setString(1, writer);
	...
	public TempMyrecipeDto getTempWriteData(String writer) {
		//글하나의 정보를 담을 BoardDto 
		TempMyrecipeDto dto=null;
		//필요한 객체의 참조값을 담을 지역변수 만들기 
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			...
			//결과 값 추출하기 
			if (rs.next()) {
				dto=new TempMyrecipeDto();
				dto.setNum(rs.getInt("tmp_num"));
				dto.setTitle(rs.getString("tmp_title"));
				dto.setSubTitle(rs.getString("tmp_subTitle"));
				dto.setContent(rs.getString("tmp_content"));
				dto.setWriter(rs.getString("tmp_writer"));
				dto.setRegdate(rs.getString("tmp_regdate"));
				dto.setImagePath(rs.getString("tmp_imagePath"));
				dto.setShowImage(rs.getString("tmp_showImage"));
				dto.setViewCount(rs.getInt("tmp_viewCount"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	//대표 이미지의 경로와 실제 이름을 DB에 저장하는 메소드
	public boolean showImage_update(MyrecipeDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql 문 준비하기
			String sql = "UPDATE my_recipe"
					+ " SET imagePath=?, showImage=?"
					+ " WHERE writer=?";
			pstmt = conn.prepareStatement(sql);
			//? 에 바인딩 할 값이 있으면 바인딩한다.
			pstmt.setString(1, dto.getImagePath());
			pstmt.setString(2, dto.getShowImage());
