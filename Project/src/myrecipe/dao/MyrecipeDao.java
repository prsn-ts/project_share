	...
	public List<MyrecipeDto> getList(MyrecipeDto dto){
		...
		try {
			...
			String sql="SELECT *"
					...
					+ " FROM"
					+ " (SELECT num,title,subTitle,content,writer,TO_CHAR(regdate,'YYYY-MM-DD') AS regdate,imagePath,showImage,viewCount"
					+ " FROM my_recipe"
	...
	public List<MyrecipeDto> getListTC(MyrecipeDto dto){
		...
		try {
			...
			String sql = "SELECT *"
					+ " FROM"
					+ "     (SELECT result1.*, ROWNUM AS rnum"
					+ "      FROM (SELECT num,title,subTitle,content,writer,TO_CHAR(regdate,'YYYY-MM-DD') AS regdate,imagePath,showImage,viewCount"
					+ "            FROM my_recipe"
					+ "            WHERE title LIKE '%'||?||'%' OR content LIKE '%'||?||'%'"
					+ "            ORDER BY num DESC) result1)"
					+ " WHERE rnum BETWEEN ? AND ?";
	...
	public boolean insert(MyrecipeDto dto) {
		...
		try {
			...
			String sql = "INSERT INTO my_recipe"
					...
					+ " VALUES(my_recipe_seq.NEXTVAL, ?, ?, ?, ?, SYSDATE, ?, ?, 0)";
			pstmt = conn.prepareStatement(sql);
			//? 에 바인딩 할 값이 있으면 바인딩한다.
			pstmt.setString(1, dto.getTitle());
